...
#     sudo installer -verbose -pkg "SDRplayAPI-macos-installer-universal-${sdrplay_api_version}.pkg" -target /
    7z x "SDRplayAPI-macos-installer-universal-${sdrplay_api_version}.pkg"
    7z x "Payload~"
    cmake_config_args+=(
        -DLIBSDRPLAY_INCLUDE_DIRS="$SRC_DIR/sdrplay_api/Library/SDRplayAPI/${sdrplay_api_version}/inc"
        -DLIBSDRPLAY_LIBRARIES="$SRC_DIR/sdrplay_api/$(ls Library/SDRplayAPI/${sdrplay_api_version}/lib/libsdrplay_api.so*)"
    )
fi
